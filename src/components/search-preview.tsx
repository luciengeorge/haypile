...
type SourceKind = "x" | "github" | "pinterest" | "youtube" | "instagram" | "reddit";
...
const EXAMPLES: Example[] = [
  {
    ...
    results: [
      { kind: "link", source: "github", title: "electric-sql / electric", meta: "Link · github.com", score: "0.76" },
      {
        kind: "link",
        source: "x",
        title: "Local-first software: the essay",
        meta: "Link · inkandswitch.com",
        score: "0.66",
      },
      {
        kind: "video",
  ...
  },
  {
    ...
    results: [
      { kind: "link", source: "x", title: "Open-weight models, ranked", meta: "Link · x.com", score: "0.77" },
      {
        kind: "image",
        source: "x",
        title: "Benchmark table screenshot",
        meta: "Image · screenshot",
        score: "0.68",
  ...
  },
  {
    ...
    results: [
      ...
      { kind: "video", source: "youtube", title: "Small bedroom makeover", meta: "Video · 6:40", score: "0.6" },
    ],
  },
  {
    ...
    results: [
      ...
      { kind: "link", source: "x", title: "Weeknight miso salmon", meta: "Link · nytimes.com", score: "0.6" },
    ],
  },
  {
    ...
    results: [
      ...
      },
      {
        ...
        meta: "Link · smashingmagazine.com",
        score: "0.6",
      },
    ],
  },
  {
    ...
    results: [
      { kind: "video", source: "youtube", title: "Trunk-based vs git flow", meta: "Video · 8:05", score: "0.75" },
      {
        kind: "link",
        source: "github",
      ...
      { kind: "link", source: "x", title: "Stop rebasing (hot take)", meta: "Link · x.com", score: "0.59" },
    ],
  },
];
...
// Simple Icons brand marks (24x24, single path), rendered monochrome as a small corner badge.
const SOURCE_PATHS: Record<SourceKind, string> = {
...
function ResultThumb({ result }: { result: Result }) {
  return (
    <div className="relative shrink-0">
      <ThumbBase result={result} />
      <div className="absolute -right-1 -bottom-1 flex size-4.5 items-center justify-center rounded-[6px] bg-card text-muted-foreground shadow-sm ring-1 ring-foreground/10">
        <svg
          viewBox="0 0 24 24"
          fill="currentColor"
          role="img"
          aria-label={`Saved from ${SOURCE_LABEL[result.source]}`}
          className="size-2.5"
        >
          <path d={SOURCE_PATHS[result.source]} />
        </svg>
      </div>
    </div>
  );
...
function ThumbBase({ result }: { result: Result }) {
  if (result.kind === "video") {
    return (
      <div className="flex size-11.5 items-center justify-center rounded-[9px] bg-foreground text-primary-foreground">
  ...
  if (result.kind === "link") {
    return (
      <div className="flex size-11.5 items-center justify-center rounded-[9px] bg-muted text-muted-foreground">
        <svg
          viewBox="0 0 24 24"
          fill="none"
          stroke="currentColor"
  ...
  return (
    <div className={cn("size-11.5 rounded-[9px] bg-gradient-to-br", result.tone ?? "from-gold to-foreground/60")} />
  );
}
