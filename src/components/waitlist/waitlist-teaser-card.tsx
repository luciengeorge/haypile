...
export function WaitlistTeaserCard() {
  ...
  return (
    <div className="flex w-full max-w-[37.5rem] flex-col gap-3.5">
      <div className="flex w-full flex-col gap-3.5 rounded-[20px] border border-foreground/6 bg-card p-5 shadow-2xl shadow-foreground/20">
...
function ResultThumb({ result }: { result: Result }) {
  ...
  return (
    <div
      className={`size-11.5 shrink-0 rounded-[9px] bg-gradient-to-br ${result.tone ?? "from-gold to-foreground/60"}`}
    />
  );
}
